...
# Configuration: set your GitHub username and token via environment variables
...
# Token must be provided via environment (never commit tokens)
GITHUB_TOKEN="${GITHUB_TOKEN:-}"

if [ -z "$GITHUB_TOKEN" ]; then
    echo "Error: GITHUB_TOKEN is not set"
    echo "Usage: GITHUB_USERNAME=your_username GITHUB_TOKEN=your_token bash setup_github.sh"
    exit 1
fi

if [ "$GITHUB_USERNAME" = "YOUR_GITHUB_USERNAME" ]; then
    echo "Error: Please set GITHUB_USERNAME to your GitHub username"
    echo "Usage: GITHUB_USERNAME=your_username GITHUB_TOKEN=your_token bash setup_github.sh"
    exit 1
fi
