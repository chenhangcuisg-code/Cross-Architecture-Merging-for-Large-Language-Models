...
try:
    ...
    for row in rows:
        ...
        if row_alpha == alpha and row_lr == lr:
            if row_variant in [model_variant, "fused", "hot", "nohot"]:
...
echo "提取hot模型结果..."
extract_malay_results \
  "/home/chenhang/MalayMMLU/output/llama3.2-1bma-instruct-fixed-nolora-sameparam_final_nolora_same_param_final_nolora_same_param_qwen/alpha0.2_lr1e-6/hot" \
  "malay" \
  "alpha0.2_lr1e-6" \
  "hot" \
  "/data/chenhang/optimal_trans_new/hot_sft_runs_llamamala_1b/llama_malaysian_sft_train_alpha_lr_search_final_nolora_same_param_qwen/alpha0.2_lr1e-6/hot" \
  "original"

# 提取nohot结果
echo "提取nohot模型结果..."
extract_malay_results \
  "/home/chenhang/MalayMMLU/output/llama3.2-1bma-instruct-fixed-nolora-sameparam_final_nolora_same_param_final_nolora_same_param_qwen/alpha0.2_lr1e-6/nohot" \
  "malay" \
  "alpha0.2_lr1e-6" \
  "nohot" \
  "/data/chenhang/optimal_trans_new/hot_sft_runs_llamamala_1b/llama_malaysian_sft_train_alpha_lr_search_final_nolora_same_param_qwen/alpha0.2_lr1e-6/nohot" \
  "original"
