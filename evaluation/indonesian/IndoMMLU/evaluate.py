...
def main():
    ...
    # Build default SAVE_FILE (f-string!)
    base_name = args.base_model.rstrip("/").split("/")[-1]
    SAVE_FILE = f"{args.output_folder}/result_{base_name}_{args.by_letter}.csv"
    ...
    if 'mt0' in args.base_model:
        model = AutoModelForSeq2SeqLM.from_pretrained(
            args.base_model, 
            device_map="auto", 
            load_in_8bit="xxl" in args.base_model
        )
    ...
    if args.lora_weights != "x":
        ...
        SAVE_FILE = f"{args.output_folder}/result_{lora_name}_{args.by_letter}.csv"
    ...
    for idx in tqdm(range(len(inputs))):
        if not args.by_letter:
            ...
            conf, pred = predict_classification_by_letter(
                model, tokenizer, inputs[idx], outputs_options[idx], device
            )
            probs.append(conf)
            preds.append(pred)
